...
with st.sidebar:
    ...
    model_choice = st.selectbox("Select Model", ["GPT-3.5", "LLaMA (Not Used)", "Other Models"])
    ...
    if st.button("→", key="submit_button", help="Submit your query"):
        if user_query:
            if model_choice == "GPT-3.5":
                response = generate_response_gpt(user_query)  # Generate response from GPT-3.5
            elif model_choice == "LLaMA (Not Used)":
                # Placeholder for LLaMA integration (not implemented)
                response = "LLaMA model not yet integrated."
            else:
